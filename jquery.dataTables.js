...
(/** @lends <global> */function($, window, document, undefined) {
			...
			if ( oSettings.oFeatures.bSort !== false )
			{
				...
				for ( i=0 ; i<aaSort.length ; i++ )
				{
					...
					for ( j=0 ; j<aDataSort.length ; j++ )
					{
						...
						//meme = meme + '&'+'iSortCol_'+iCounter+'='+aDataSort[j]+'&'+'sSortDir_'+iCounter+'='+aaSort[i][1];
						iCounter++;
					}
				}
				aoData.push( { "name": "iSortingCols",   "value": iCounter } );
				//meme = meme + '&'+"iSortingCols="+iCounter;
				
				for ( i=0 ; i<iColumns ; i++ )
				{
					aoData.push( { "name": "bSortable_"+i,  "value": oSettings.aoColumns[i].bSortable } );
					//meme = meme + '&'+"bSortable_"+i+'='+oSettings.aoColumns[i].bSortable;
				}
			}
			//alert(meme);
			return aoData;
	...
	DataTable.defaults = {
		...
		"fnServerData": function ( sUrl, aoData, fnCallback, oSettings ) {
			...
			if( oSettings && oSettings.oInit && oSettings.oInit.more_data && ! $.isEmptyObject( oSettings.oInit.more_data ) ){
				aoData.push( {'name': 'more_data', 'value': oSettings.oInit.more_data } );
			}
			
			var $d = aoData;
			if( oSettings.sServerMethod == 'POST' ){
				$d = {'dt': JSON.stringify( aoData ) };
			}
	 ...
	 * y default DataTables only uses the sAjaxSource variable at initialisation time, however it can be useful to re-read an Ajax source and have the table *update. Typically you would need to use the fnClearTable() and fnAddData() functions, however this wraps it all up in a single function call.
